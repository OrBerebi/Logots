...
def get_visual_model(device: str = 'mps'): 
    ...
    with _yolo_lock:
        if _yolo_model is None:
            ...
            model_path = os.path.join(os.getcwd(), './src/logots/models/', 'yolov8m.pt')

            if not os.path.exists(model_path):
                print(f"⚠️ Model not found at {model_path}. YOLO will attempt to download it...")
                # We initialize with the name to trigger download, then we might need to move it manually
                # or just let it download. simpler approach:
                model = YOLO('yolov8m.pt') # Downloads to root if missing
                # If you want to force it to 'models/', you usually have to move it after download 
                # or download it manually. 
                # BUT, if you already moved the file to 'models/', this line works:
