...
MOTORS_ESP32_PORT = 12345
VIDEO_ESP32_CAM_IP_SET_RES = "http://192.168.178.100"
...
def stream_motors(stop_event, buffer):
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            # B. Arbitration Logic
            # Case 1: Manual Wheel Override
            # If user is driving wheels, they have absolute priority.
            ...
            # Case 2: Auto Pilot (Brain)
            # If wheels are idle, check if the brain wants to do something
            elif not BRAIN_COMMAND_QUEUE.empty():
                final_l, final_r, final_a = BRAIN_COMMAND_QUEUE.get()
                
            # Case 3: Idle / Manual Arm Only
            else:
                # FIX: Stop wheels, but RESPECT the Manual Arm Angle
                final_l, final_r = 0, 0
                final_a = manual_a 
